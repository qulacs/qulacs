...
PYBIND11_MODULE(qulacs, m) {
	...
	py::class_<GeneralQuantumOperator>(m, "GeneralQuantumOperator")
		...
		.def(
			"get_term", [](const GeneralQuantumOperator &quantum_operator, const unsigned int index) {
	...
	py::class_<HermitianQuantumOperator, GeneralQuantumOperator>(m, "Observable")
		...
		.def(
			"get_term", [](const HermitianQuantumOperator &quantum_operator, const unsigned int index) {
				return quantum_operator.get_term(index)->copy();
			},
			pybind11::return_value_policy::take_ownership, "Get Pauli term", py::arg("index"))
		// .def("get_expectation_value", &HermitianQuantumOperator::get_expectation_value)
		.def(
			"get_expectation_value", [](const HermitianQuantumOperator &observable, const QuantumStateBase *state) {
	...
	py::class_<QuantumState, QuantumStateBase>(m, "QuantumState")
		...
		.def(
			"get_vector", [](const QuantumState &state) {
				Eigen::VectorXcd vec = Eigen::Map<Eigen::VectorXcd>(state.data_cpp(), state.dim);
				return vec;
			},
			"Get state vector")
		.def(
			"get_qubit_count", [](const QuantumState &state) -> unsigned int { return (unsigned int)state.qubit_count; }, "Get qubit count")
	...
	py::class_<DensityMatrix, QuantumStateBase>(m, "DensityMatrix")
		...
		.def(
			"get_matrix", [](const DensityMatrix &state) {
	...
	py::class_<QuantumStateGpu, QuantumStateBase>(m, "QuantumStateGpu")
		...
		.def(
			"get_vector", [](const QuantumStateGpu &state) {
		...
		.def(
			"get_qubit_count", [](const QuantumStateGpu &state) -> unsigned int { return (unsigned int)state.qubit_count; }, "Get qubit count")
	...
	py::class_<QuantumGateBase>(m, "QuantumGateBase")
		...
		.def(
			"get_matrix", [](const QuantumGateBase &gate) {
				ComplexMatrix mat;
				gate.set_matrix(mat);
				return mat;
			},
	...
	py::class_<QuantumGateMatrix, QuantumGateBase>(m, "QuantumGateMatrix")
		...
		.def(
			"get_matrix", [](const QuantumGateMatrix &gate) {
				ComplexMatrix mat;
				gate.set_matrix(mat);
				return mat;
			},
	...
	mgate.def(
		"CNOT", [](UINT control_qubit_index, UINT target_qubit_index) {
	...
	mgate.def(
		"CZ", [](UINT control_qubit_index, UINT target_qubit_index) {
	...
	mgate.def(
		"SWAP", [](UINT target_index1, UINT target_index2) {
	...
	mgate.def(
		"Pauli", [](std::vector<unsigned int> target_qubit_index_list, std::vector<unsigned int> pauli_ids) {
			...
			if (ptr == NULL)
				throw std::invalid_argument("Invalid argument passed to Pauli.");
			return ptr;
		},
		pybind11::return_value_policy::take_ownership, "Create multi-qubit Pauli gate", py::arg("index_list"), py::arg("pauli_ids"));
	mgate.def(
		"PauliRotation", [](std::vector<unsigned int> target_qubit_index_list, std::vector<unsigned int> pauli_ids, double angle) {
			if (target_qubit_index_list.size() != pauli_ids.size())
				throw std::invalid_argument("Size of qubit list and pauli list must be equal.");
			auto ptr = gate::PauliRotation(target_qubit_index_list, pauli_ids, angle);
			if (ptr == NULL)
				throw std::invalid_argument("Invalid argument passed to PauliRotation.");
			return ptr;
		},
	...
	mgate.def(
		"DenseMatrix", [](unsigned int target_qubit_index, ComplexMatrix matrix) {
			if (matrix.rows() != 2 || matrix.cols() != 2)
				throw std::invalid_argument("matrix dims is not 2x2.");
			auto ptr = gate::DenseMatrix(target_qubit_index, matrix);
			if (ptr == NULL)
				throw std::invalid_argument("Invalid argument passed to DenseMatrix.");
			return ptr;
		},
		pybind11::return_value_policy::take_ownership, "Create dense matrix gate", py::arg("index"), py::arg("matrix"));
	mgate.def(
		"DenseMatrix", [](std::vector<unsigned int> target_qubit_index_list, ComplexMatrix matrix) {
	...
	mgate.def(
		"SparseMatrix", [](std::vector<unsigned int> target_qubit_index_list, SparseComplexMatrix matrix) {
			const ITYPE dim = 1ULL << target_qubit_index_list.size();
			if (matrix.rows() != dim || matrix.cols() != dim)
				throw std::invalid_argument("matrix dims is not consistent.");
			auto ptr = gate::SparseMatrix(target_qubit_index_list, matrix);
			if (ptr == NULL)
				throw std::invalid_argument("Invalid argument passed to SparseMatrix.");
			return ptr;
		},
		pybind11::return_value_policy::take_ownership, "Create sparse matrix gate", py::arg("index_list"), py::arg("matrix"));

	mgate.def(
		"RandomUnitary", [](std::vector<unsigned int> target_qubit_index_list) {
			auto ptr = gate::RandomUnitary(target_qubit_index_list);
			if (ptr == NULL)
				throw std::invalid_argument("Invalid argument passed to RandomUnitary.");
			return ptr;
		},
		pybind11::return_value_policy::take_ownership, "Create random unitary gate", py::arg("index_list"));
	mgate.def(
		"ReversibleBoolean", [](std::vector<UINT> target_qubit_list, std::function<ITYPE(ITYPE, ITYPE)> function_py) {
			auto ptr = gate::ReversibleBoolean(target_qubit_list, function_py);
			if (ptr == NULL)
				throw std::invalid_argument("Invalid argument passed to ReversibleBoolean.");
			return ptr;
		},
		pybind11::return_value_policy::take_ownership, "Create reversible boolean gate", py::arg("index_list"), py::arg("func"));
	mgate.def(
		"StateReflection", [](const QuantumStateBase *reflection_state) {
			auto ptr = gate::StateReflection(reflection_state);
			if (ptr == NULL)
				throw std::invalid_argument("Invalid argument passed to StateReflection.");
			return ptr;
		},
	...
	mgate.def(
		"TwoQubitDepolarizingNoise", [](UINT target_index1, UINT target_index2, double probability) {
			auto ptr = gate::TwoQubitDepolarizingNoise(target_index1, target_index2, probability);
			if (ptr == NULL)
				throw std::invalid_argument("Invalid argument passed to TwoQubitDepolarizingNoise.");
			return ptr;
		},
	...
	mgate.def(
		"ParametricPauliRotation", [](std::vector<unsigned int> target_qubit_index_list, std::vector<unsigned int> pauli_ids, double angle) {
			auto ptr = gate::ParametricPauliRotation(target_qubit_index_list, pauli_ids, angle);
			if (ptr == NULL)
				throw std::invalid_argument("Invalid argument passed to ParametricPauliRotation.");
			return ptr;
		},
	...
	py::class_<QuantumCircuit>(m, "QuantumCircuit")
		...
		.def(
			"get_gate", [](const QuantumCircuit &circuit, unsigned int index) -> QuantumGateBase * {
		...
		.def(
			"get_gate_count", [](const QuantumCircuit &circuit) -> unsigned int { return (unsigned int)circuit.gate_list.size(); }, "Get gate count")
		.def(
			"get_qubit_count", [](const QuantumCircuit &circuit) -> unsigned int { return circuit.qubit_count; }, "Get qubit count")
