...
void Google_noisy_random_circuit(int length, int depth, QuantumCircuit *ans,
                                 double prob) {
  int n = length * length;
  for (int k = 0; k < depth; ++k) {
    const int dx[4] = {1, -1, 0, 0};
      ...
      for (int i = 0; i < length; ++i) {
        for (int j = 0; j < length; ++j) {
          if ((i + j) % 2 == 0) {
            ...
            if (x >= 0 && x < length && y >= 0 && y < length) {
              grid_2Q_gate({(UINT)i * length + j, (UINT)(x)*length + y}, ans);
              ans->add_gate(gate::TwoQubitDepolarizingNoise(
                  i * length + j, x * length + y, prob));
            }
          }
        }
      }
...
int main(int argc, char **argv) {
...

  {
    ...
    auto dur = end - start;
    auto msec =
        dur; // std::chrono::duration_cast<std::chrono::milliseconds>(dur).count();
    if (myrank == 0) {
