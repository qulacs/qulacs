...
int main() {
    ...
    std::function<double(
        UINT, double, UINT, std::function<void(UINT, Timer &)>)>
        benchmark_func = [](UINT n, double timeout_, UINT max_repeat_,
    ...
    std::function<double(UINT, double, UINT,
        std::function<void(UINT, StateVectorCpu &, Timer &)>)>
        benchmark_state_ops_func =
            [](UINT n, double timeout_, UINT max_repeat_,
                std::function<void(UINT, StateVectorCpu &, Timer &)> func) {
                UINT repeat = 0;
                Timer timer;
                timer.reset();
                timer.temporal_stop();
                ...
                ITYPE dim = 1ULL << n;
                StateVectorCpu state = StateVectorCpu(n);
                do {
                    func(n, state, timer);
                    repeat++;
                } while (timer.elapsed() < timeout_ && repeat <= max_repeat_);
    ...
    for (UINT qubit_count = min_qubit_count; qubit_count < max_qubit_count;
        ...
        for (UINT k = 1; k <= max_dense_qubit_count; ++k) {
            ...
            ComplexMatrix matrix =
                ComplexMatrix::Identity(matrix_dim, matrix_dim);
            int n = qubit_count;
            ...
            for (int i = 0; i < max_repeat; ++i) {
                QuantumGateBasic *tmp =
                    QuantumGateBasic::DenseMatrixGate(targets, matrix);
                circuit.add_gate(tmp);
            }
            Timer timer;
            timer.reset();
