...
void test_single_qubit_named_gate(UINT n, std::string name, std::function<void(UINT, void*, ITYPE)> func, Eigen::MatrixXcd mat) {
	const ITYPE dim = 1ULL << n;
	const UINT max_repeat = 2;

	auto state = allocate_quantum_state_host(dim);
	initialize_Haar_random_state_with_seed_host(state, dim, 0);
	...
	for (UINT rep = 0; rep < max_repeat; ++rep) {
		for (UINT i = 0; i < n; ++i) {
			...
			state_equal_gpu(state, test_state, dim, name);
		}
...
TEST(UpdateTest, XGate) {
	...
	mat << 0, 1, 1, 0;
	void(*func)(UINT, void*, ITYPE) = &X_gate_host;
	test_single_qubit_named_gate(6, "XGate", func, mat);
}
TEST(UpdateTest, YGate) {
	Eigen::MatrixXcd mat(2, 2);
	mat << 0, -1.i, 1.i, 0;
	void(*func)(UINT, void*, ITYPE) = &Y_gate_host;
...
TEST(UpdateTest, ZGate) {
	...
	Eigen::MatrixXcd mat(2, 2);
	mat << 1, 0, 0, -1;
	void(*func)(UINT, void*, ITYPE) = &Z_gate_host;
...
TEST(UpdateTest, HGate) {
	...
	void(*func)(UINT, void*, ITYPE) = &H_gate_host;
...
TEST(UpdateTest, SGate) {
	...
	Eigen::MatrixXcd mat(2, 2);
	mat << 1, 0, 0, 1.i;
	void(*func)(UINT, void*, ITYPE) = &S_gate_host;
	test_single_qubit_named_gate(n, "SGate", func, mat);
	void(*func2)(UINT, void*, ITYPE) = &Sdag_gate_host;
...
TEST(UpdateTest, TGate) {
	...
	void(*func)(UINT, void*, ITYPE) = &T_gate_host;
	test_single_qubit_named_gate(n, "TGate", func, mat);
	void(*func2)(UINT, void*, ITYPE) = &Tdag_gate_host;
...
TEST(UpdateTest, sqrtXGate) {
	...
	void(*func)(UINT, void*, ITYPE) = &sqrtX_gate_host;
	test_single_qubit_named_gate(n, "SqrtXGate", func, mat);
	void(*func2)(UINT, void*, ITYPE) = &sqrtXdag_gate_host;
...
TEST(UpdateTest, sqrtYGate) {
	...
	Eigen::MatrixXcd mat(2, 2);
	mat << 0.5 + 0.5i, -0.5 - 0.5i, 0.5 + 0.5i, 0.5 + 0.5i;
	void(*func)(UINT, void*, ITYPE) = &sqrtY_gate_host;
	test_single_qubit_named_gate(n, "SqrtYGate", func, mat);
	void(*func2)(UINT, void*, ITYPE) = &sqrtYdag_gate_host;
	test_single_qubit_named_gate(n, "SqrtYGate", func2, mat.adjoint());
}

void test_projection_gate(std::function<void(UINT, void*, ITYPE)> func, std::function<double(UINT, void*, ITYPE)> prob_func, Eigen::MatrixXcd mat) {
	const UINT n = 6;
	const ITYPE dim = 1ULL << n;
	...
	for (UINT rep = 0; rep < max_repeat; ++rep) {
		...
		func(target, state, dim);
		ASSERT_NEAR(state_norm_squared_host(state, dim), prob, eps);
		normalize_host(prob, state, dim);

		test_state = get_expanded_eigen_matrix_with_identity(target, mat, n)*test_state;
		ASSERT_NEAR(test_state.squaredNorm(), prob, eps);
		test_state.normalize();
		state_equal_gpu(state, test_state, dim, "Projection gate");
	}
	release_quantum_state_host(state);
}
...
TEST(UpdateTest, ProjectionAndNormalizeTest) {
	...
	P0 << 1, 0, 0, 0;
	P1 << 0, 0, 0, 1;
	void(*func1)(UINT, void*, ITYPE) = &P0_gate_host;
	double(*func2)(UINT, void*, ITYPE) = &M0_prob_host;
...
TEST(UpdateTest, SingleQubitRotationGateTest) {
	...
	initialize_Haar_random_state_host(state, dim);
	Eigen::VectorXcd test_state = copy_cpu_from_gpu(state,dim);
	typedef std::tuple<std::function<void(UINT, double, void*, ITYPE)>, Eigen::MatrixXcd, std::string> testset;
	std::vector<testset> test_list;
	void(*func)(UINT, double, void*, ITYPE) = &RX_gate_host;
...
void test_two_qubit_named_gate(UINT n, std::string name, std::function<void(UINT, UINT, void*, ITYPE)> func,
