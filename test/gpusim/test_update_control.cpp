...
void test_single_control_single_target(std::function<void(unsigned int, unsigned int, unsigned int, const CPPCTYPE*, void*, ITYPE)> func) {
    const UINT n = 6;
...
TEST(UpdateTest, SingleQubitControlSingleQubitDenseMatrixTest) {
	void(*func)(UINT, UINT, UINT, const CPPCTYPE*, void*, ITYPE) 
...
TEST(UpdateTest, TwoQubitControlTwoQubitDenseMatrixTest) {
    ...
    for (UINT rep = 0; rep < max_repeat; ++rep) {
        ...
        state_equal_gpu(state, test_state, dim, "two qubit control two qubit dense gate");
    }
    release_quantum_state_host(state);
}
