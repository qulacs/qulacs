...
TEST(StateTest, AddState) {
    const double eps = 1e-10;
    const UINT n = 10;
    StateVector state1(n);
    StateVector state2(n);
...
TEST(StateTest, TensorProduct) {
    ...
    StateVector state1(n), state2(n);
    state1.set_Haar_random_state();
    state2.set_Haar_random_state();

    StateVector* state3 = state::tensor_product(&state1, &state2);
...
TEST(StateTest, DropQubit) {
    ...
    StateVector state(n);
    state.set_Haar_random_state();
    StateVector* state2 = state::drop_qubit(&state, {2, 0}, {0, 1});
...
TEST(StateTest, PermutateQubit) {
    ...
    StateVector state(n);
    state.set_Haar_random_state();
    StateVector* state2 = state::permutate_qubit(&state, {1, 0, 2});
