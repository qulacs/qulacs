...
TEST(SingleFermionOperatorTest, InitTest) {
    ...
    for (int i = 0; i < 4; i++) {
        EXPECT_EQ(op1.get_target_index_list().at(i),
            op2.get_target_index_list().at(i));
        EXPECT_EQ(
            op1.get_action_id_list().at(i), op2.get_action_id_list().at(i));
    }
}
...
TEST(SingleFermionOperatorTest, operatorTest) {
    ...
    for (int i = 0; i < 4; i++) {
        EXPECT_EQ(op3.get_target_index_list().at(i),
            expected.get_target_index_list().at(i));
        EXPECT_EQ(op3.get_action_id_list().at(i),
            expected.get_action_id_list().at(i));
    }
    ...
    for (int i = 0; i < 4; i++) {
        EXPECT_EQ(op1.get_target_index_list().at(i),
            expected.get_target_index_list().at(i));
        EXPECT_EQ(op1.get_action_id_list().at(i),
            expected.get_action_id_list().at(i));
    }
}