...
TEST(NoisyEvolutionTest, EffectiveHamiltonian) {
    ...
    auto gate = dynamic_cast<ClsNoisyEvolution*>(gate::NoisyEvolution(&hamiltonian, c_ops, time, dt));
    ASSERT_EQ(gate->get_effective_hamiltonian()->to_string(), "(1,0) Z 0 Z 1 + (0,-1) I");
}
...
TEST(NoisyEvolutionTest, dephasing) {
    ...
    double ref_withoutnoise = 0.696706573268861; //generated by qutip (needed for variance calculation)
    UINT n = 2;
    UINT n_samples = 1000;
    Observable observable(n);
    observable.add_operator(1, "X 0");
    ...
    // take 5-sigma for assertion. Correct code should violate this assertion by probabilty of only 3e-4 %.
    auto fivesigma = 5*sqrt(ref_withoutnoise*ref_withoutnoise-ref*ref)/sqrt(n_samples);
...
TEST(NoisyEvolutionTest, check_inf_occurence) {
    ...
    for (int k=0; k<n_samples; k++){
        circuit.update_quantum_state(&state);
        ASSERT_FALSE(std::isinf(observable.get_expectation_value(&state).real()));
    }
}