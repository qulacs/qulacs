...
TEST(NoisyEvolutionTest_auto, tekitouu) {
    // 2 qubit dephasing dynamics with ZZ interaction
    double time = 0.8;
    double dt = 0.1;
    double decay_rate_z = 0.2;
    ...
    //[014][2] に2分割され、3は未使用なのが必要
    Observable observable(n);
    observable.add_operator(1, "X 0");
    observable.add_operator(1, "X 2");
    // create hamiltonian and collapse operator
    Observable hamiltonian(n);
    hamiltonian.add_operator(hamiltonian_energy, "Z 0 Z 4");
    hamiltonian.add_operator(0.5, "X 1");
    hamiltonian.add_operator(0.5, "Y 2");
    std::vector<GeneralQuantumOperator*> c_ops;
    for (int k = 0; k < 2; k++) c_ops.push_back(new GeneralQuantumOperator(n));
    c_ops[0]->add_operator(decay_rate_z, "Z 2");
    c_ops[1]->add_operator(0.1, "X 0 Z 1");
    c_ops[1]->add_operator(0.1 * 1.i, "Y 0 Y 1");
    QuantumState state(n);
    QuantumCircuit circuit(n);
    auto Nagate = gate::NoisyEvolution_auto(&hamiltonian, c_ops, time);
    circuit.add_gate(Nagate);
    QuantumCircuit circuit_old(n);
    circuit_old.add_gate(gate::NoisyEvolution_fast(&hamiltonian, c_ops, time));
    double exp = 0.;
    double exp_old = 0.;
    for (int k = 0; k < n_samples; k++) {
        state.set_zero_state();
