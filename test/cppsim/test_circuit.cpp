...
TEST(CircuitTest, CircuitBasic) {
	...
	for (ITYPE i = 0; i < dim; ++i)
		state_eigen[i] = state.data_cpp()[i];
			...
			for (UINT i = 0; i < n; ++i) {
				...
				if (r == 0)
					...
					if (r2 == i)
						r2 = (r2 + 1) % n;
					if (i + 1 < n)
						circuit.add_CNOT_gate(i, r2);
				} else if (r == 4) {
					UINT r2 = random.int32() % n;
					if (r2 == i)
						r2 = (r2 + 1) % n;
					if (i + 1 < n)
						circuit.add_CZ_gate(i, r2);
				} else if (r == 5) {
					UINT r2 = random.int32() % n;
					if (r2 == i)
						r2 = (r2 + 1) % n;
					if (i + 1 < n)
						circuit.add_SWAP_gate(i, r2);
				}
			}
		}
