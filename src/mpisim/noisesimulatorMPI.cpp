...
std::vector<UINT> NoiseSimulatorMPI::execute(const UINT sample_count) {
    ...
    if (myrank == 0) {
        ...
        for (UINT i = 0; i < sample_count; ++i) {
            ...
            if (i + 1 == sample_count or trial_gates[i] != trial_gates[i + 1]) {
                ...
                sampling_required_all.push_back(
                    std::make_pair(now, cnter_samplings));
                cnter_samplings = 0;
            }
        }
        cnter /= numprocs;
    ...
    // now sampling required to perform in each node is in
    // sampling_required_rec. lets transform  vector<UINT> into
    // vector<pair<vector<UINT>,UINT>> again.

    sampling_required_thisnode.resize(
        sampling_required_rec.size() / OneSampling_DataSize);
    for (int i = 0; i < sampling_required_rec.size(); ++i) {
        if (i % OneSampling_DataSize == OneSampling_DataSize - 1) {
            ...
            sampling_required_thisnode[i / OneSampling_DataSize]
                .first.push_back(sampling_required_rec[i]);
        }
    }
