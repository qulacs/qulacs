    ...
    for (i = 0; i < 2; ++i)
        for (j = 0; j < 2; ++j)
            rotation_gate[i * 2 + j] =
                ...
                sin(angle / 2) * 1.0i *
                    PAULI_MATRIX[Pauli_operator_index][i * 2 + j];
...
#pragma omp parallel for
    ...
    for (state_index = 0; state_index < loop_dim; ++state_index) {
        ...
        ITYPE basis_0 = insert_zero_to_basis_index(
            state_index, target_mask, target_qubit_index);
...
#pragma omp parallel for
    ...
    for (state_index = 0; state_index < loop_dim; ++state_index) {
        ...
        basis_c_t0 = insert_zero_to_basis_index(
            basis_c_t0, min_qubit_mask, min_qubit_index);
        basis_c_t0 = insert_zero_to_basis_index(
            basis_c_t0, max_qubit_mask, max_qubit_index);
...
// This function can be further optimized by disucssing what should be computed
// before loop as local object. Current function is designed to avoid
    ...
    for (state_index = 0; state_index < loop_dim; ++state_index) {
        ...
        for (UINT cursor = 0; cursor < insert_index_list_count; ++cursor) {
            basis_c_t0 = insert_zero_to_basis_index(basis_c_t0,
                1ULL << insert_index_list[cursor], insert_index_list[cursor]);
        }
