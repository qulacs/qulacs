...
public:
    ...
    virtual void update_quantum_state(QuantumStateBase* state) {
        ...
        while (std::abs(t - _time)>1e-10*_time){ // for machine precision error
            ...
            if (norm <= r){
                ...
                auto dt_target_norm = _find_collapse(k1,k2,k3,k4,buffer,state,r,dt);
    ...
    /**
     * \~japanese-en collapse が起こるタイミング (norm = rになるタイミング) を見つける。
     * この関数内で norm=rになるタイミングまでの evolution が行われる。
     */
    virtual double _find_collapse(
        ...
        QuantumStateBase* k3,
        QuantumStateBase* k4,
        QuantumStateBase* prev_state,
        QuantumStateBase* now_state,
        double target_norm,
        double dt
    ){
        ...
        auto t_guess = dt;
        int search_count = 0;
        // std::cout << "prev norm" << prev_state->get_squared_norm() << std::endl;
        // std::cout << "now state: " << now_norm << " prev state: " << prev_norm << target_norm << std::endl;
        while (std::abs(now_norm-target_norm) > _norm_tol) {
            // we expect norm to reduce as Ae^-a*dt, so we first calculate the coefficient a
            auto a = std::log(now_norm/prev_norm)/t_guess; 
            // then guess the time to become target norm as (target_norm) = (prev_norm)e^-a*(t_guess)
            // which means -a*t_guess = log(target_norm/prev_norm)
     ...
     */
    virtual void _evolve_one_step(
        QuantumStateBase* k1,
        QuantumStateBase* k2,
        QuantumStateBase* k3,
        QuantumStateBase* k4,
        QuantumStateBase* buffer,
