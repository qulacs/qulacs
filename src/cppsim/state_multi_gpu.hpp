...
class QuantumStateMultiGpu : public QuantumStateBase {
private:
...
public:
	 ...
	 */
	QuantumStateMultiGpu(UINT qubit_count_) : QuantumStateBase(qubit_count_, true, 0) {
        set_device(0);
		this->_cuda_stream = allocate_cuda_stream_host(1);
    ...
    QuantumStateMultiGpu(UINT qubit_count_, UINT device_number) : QuantumStateBase(qubit_count_, true, device_number) {
	 ...
	 * \~japanese-en デストラクタ
	 */
	virtual ~QuantumStateMultiGpu() {
		release_quantum_state_host(this->data(), device_number);
		release_cuda_stream_host(this->_cuda_stream, 1);
	}
	/**
	...
	virtual void set_computational_basis(ITYPE comp_basis)  override {
        set_computational_basis_host(comp_basis, _state_vector, _dim, device_number);
	}
	/**
	...
	virtual QuantumStateBase* allocate_buffer() const override {
		QuantumStateMultiGpu* new_state = new QuantumStateMultiGpu(this->_qubit_count, device_number);
		return new_state;
	}
	/**
	...
	virtual QuantumStateBase* copy() const override {
		QuantumStateMultiGpu* new_state = new QuantumStateMultiGpu(this->_qubit_count, device_number);
        copy_quantum_state_from_device_to_device(new_state->data(), _state_vector, _dim, device_number);
		return new_state;
	}
	/**
	...
	virtual void load(const QuantumStateBase* _state) override {
		if (_state->get_device_name() == "multi_gpu") {
			copy_quantum_state_from_device_to_device(this->data(), _state->data(), dim, device_number);
		}
		else {
	...
	*/
	virtual void load(const std::vector<CPPCTYPE>& _state) override{
		copy_quantum_state_from_cppstate_host(this->data(), _state.data(), dim, device_number);
	}
	...
	*/
	virtual void load(const CPPCTYPE* _state) override{
		copy_quantum_state_from_cppstate_host(this->data(), _state, dim, device_number);
	}
	/**
	 * \~japanese-en 量子状態が配置されているメモリを保持するデバイス名を取得する。
	 */
	virtual const std::string get_device_name() const override { return "multi_gpu"; }
	...
	*/
	...
	virtual CTYPE* duplicate_data_c() const override {
		...
		get_quantum_state_host(this->_state_vector, _copy_state, dim, device_number);
		return _copy_state;
	}
...
namespace state {
	...
	* @return 内積の値
	*/
	CPPCTYPE DllExport inner_product(const QuantumStateMultiGpu* state_bra, const QuantumStateMultiGpu* state_ket);
	CPPCTYPE DllExport inner_product(const QuantumStateMultiGpu* state_bra, const QuantumStateGpu* state_ket);
	CPPCTYPE DllExport inner_product(const QuantumStateGpu* state_bra, const QuantumStateMultiGpu* state_ket);
}
