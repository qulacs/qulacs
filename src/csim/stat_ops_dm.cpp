...
void dm_state_add_with_coef(CTYPE coef, const CTYPE* state_added, CTYPE* state, ITYPE dim) {
    ITYPE index;
#ifdef _OPENMP
#pragma omp parallel for
#endif
...
double dm_expectation_value_multi_qubit_Pauli_operator_partial_list(
    ...
    for (ITYPE state_index = 0; state_index < dim; ++state_index) {
        ...
        for (ITYPE i = 0; i < matrix_dim; ++i) {
            sum += matrix[small_dim_index * matrix_dim + i] *
                   state[state_index * dim + (basis_0 ^ matrix_mask_list[i])];
        }
    }
