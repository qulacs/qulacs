...
UnionFind::UnionFind(int N) { Parent = std::vector<int>(N, -1); }
...
bool UnionFind::same(int A, int B) { return root(A) == root(B); }

int UnionFind::size(int A) { return -Parent[root(A)]; }

bool UnionFind::connect(int A, int B) {
    A = root(A);
    B = root(B);
    if (A == B) {
        return false;
...
CausalConeSimulator::CausalConeSimulator(
    const ParametricQuantumCircuit& _init_circuit,
...
void CausalConeSimulator::build() {
    build_run = true;
    ...
    for (auto term : terms) {
        ...
        for (int i = gate_count - 1; i >= 0; i--) {
            ...
            if (use_gate[i]) {
                ...
                for (UINT j = 0; j + 1 < (UINT)target_index_list.size(); j++) {
                    uf.connect(target_index_list[j], target_index_list[j + 1]);
                }
                for (UINT j = 0; j + 1 < (UINT)control_index_list.size(); j++) {
                    uf.connect(
...
CPPCTYPE CausalConeSimulator::get_expectation_value() {
    if (!build_run) build();
    CPPCTYPE ret;
...
std::vector<std::vector<ParametricQuantumCircuit*>>
CausalConeSimulator::get_circuit_list() {
    return circuit_list;
}

std::vector<std::vector<PauliOperator>>
CausalConeSimulator::get_pauli_operator_list() {
    return pauli_operator_list;
}

std::vector<CPPCTYPE> CausalConeSimulator::get_coef_list() { return coef_list; }
