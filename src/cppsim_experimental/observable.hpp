...
public:
    ...
    virtual const std::vector<UINT>& get_pauli_id_list() const { return _pauli_id; }
        ...
        if (state->get_device_type() == DEVICE_CPU) {
            if (state->is_state_vector()) {
                return expectation_value_multi_qubit_Pauli_operator_partial_list(
                    this->_target_index.data(),
                    this->_pauli_id.data(),
                    (UINT)this->_target_index.size(),
                    state->data_c(),
                    state->dim);
            }
            else {
            ...
            if (state->is_state_vector()) {
                ...
                throw std::runtime_error("Get expectation value for DensityMatrix on GPU is not supported");
            }
#else
    ...
    virtual CPPCTYPE get_transition_amplitude(const QuantumStateBase* state_bra, const QuantumStateBase* state_ket) const {
        ...
        if (state_bra->get_device_type() == DEVICE_CPU) {
            if (state_bra->is_state_vector()) {
                ...
                throw std::invalid_argument("TransitionAmplitude for density matrix is not implemtend");
            }
        }
            ...
            if (state->is_state_vector()) {
                return  transition_amplitude_multi_qubit_Pauli_operator_partial_list_host(
                    this->get_index_list().data(),
                    this->get_pauli_id_list().data(),
                    (UINT)this->get_index_list().size(), state_bra->data(), state_ket->data(),
                    state_bra->dim, state_bra->get_cuda_stream(),
...
public:
    ...
    virtual CPPCTYPE get_expectation_value(const QuantumStateBase* state) const {
        ...
        for (UINT index = 0; index < _pauli_terms.size(); ++index) {
            sum += _coef_list.at(index) * _pauli_terms.at(index).get_expectation_value(state);
        }
        return sum;
    }
    virtual CPPCTYPE get_transition_amplitude(const QuantumStateBase* state_bra, const QuantumStateBase* state_ket) const {
        CPPCTYPE sum = 0;
        for (UINT index = 0; index < _pauli_terms.size(); ++index) {
            sum += _coef_list.at(index) * _pauli_terms.at(index).get_transition_amplitude(state_bra, state_ket);
        }
        return sum;
    }
};
